        ...
        public static class cluster
        {
            ...
            public class HDBSCAN : PythonObject
            {
                ...
                public NDarray dbscan_clustering(float cut_distance, int min_cluster_size = 5)
                {
                    ...
                    if (min_cluster_size != 5)
                        pyDict["min_cluster_size"] = ToPython(min_cluster_size);
                    return ToCsharp<NDarray>(self.InvokeMethod("dbscan_clustering", args, pyDict));
                }
            ...
            public class SpectralBiclustering : PythonObject
            {
                ...
                public NDarray biclusters_ => ToCsharp<NDarray>(self.GetAttr("biclusters_"));
                public int n_features_in_ => ToCsharp<int>(self.GetAttr("n_features_in_"));
                public NDarray feature_names_in_ => ToCsharp<NDarray>(self.GetAttr("feature_names_in_"));
            ...
            {
                PyTuple args = ToTuple(new object[] { S });
                PyDict pyDict = new PyDict();
                if (preference != null)
                    pyDict["preference"] = ToPython(preference);
                if (convergence_iter != 15)
            ...
            {
                PyTuple args = ToTuple(new object[] { reachability, core_distances, ordering, eps });
                PyDict pyDict = new PyDict();
                return ToCsharp<NDarray>(sklearn.cluster.self.InvokeMethod("cluster_optics_dbscan", args, pyDict));
            }
            ...
            {
                PyTuple args = ToTuple(new object[] { reachability, predecessor, ordering, min_samples });
                PyDict pyDict = new PyDict();
                if (min_cluster_size != null)
                    pyDict["min_cluster_size"] = ToPython(min_cluster_size);
                if (xi != 0.05f)
            ...
            {
                PyTuple args = ToTuple(new object[] { X, min_samples, max_eps, metric, p, metric_params, algorithm, leaf_size, n_jobs });
                PyDict pyDict = new PyDict();
            ...
            public static float estimate_bandwidth(NDarray X, float quantile = 0.3f, int? n_samples = null, int? random_state = 0, int? n_jobs = null)
            {
                ...
                if (n_jobs != null)
                    pyDict["n_jobs"] = ToPython(n_jobs);
                return ToCsharp<float>(sklearn.cluster.self.InvokeMethod("estimate_bandwidth", args, pyDict));
            }
            ...
            {
                PyTuple args = ToTuple(new object[] { X, n_clusters });
                PyDict pyDict = new PyDict();
                if (sample_weight != null)
                    pyDict["sample_weight"] = ToPython(sample_weight);
                if (init != "k-means++")
                ...
                return (ToCsharp<NDarray>(result[0]), ToCsharp<NDarray>(result[1]), ToCsharp<float>(result[2]), ToCsharp<int>(result[3]));
            }
            ...
            public static (NDarray, NDarray) kmeans_plusplus(NDarray X, int n_clusters, NDarray? sample_weight = null, NDarray? x_squared_norms = null, int? random_state = null, int? n_local_trials = null)
            {
                ...
                if (x_squared_norms != null)
                    pyDict["x_squared_norms"] = ToPython(x_squared_norms);
                if (random_state != null)
                    pyDict["random_state"] = ToPython(random_state);
                if (n_local_trials != null)
                    pyDict["n_local_trials"] = ToPython(n_local_trials);
            ...
            {
                PyTuple args = ToTuple(new object[] { affinity });
                PyDict pyDict = new PyDict();
                if (n_clusters != 8)
                    pyDict["n_clusters"] = ToPython(n_clusters);
                if (n_components != null)
                ...
                if (verbose != false)
                    pyDict["verbose"] = ToPython(verbose);
                return ToCsharp<NDarray>(sklearn.cluster.self.InvokeMethod("spectral_clustering", args, pyDict));
            }
            ...
            public static (NDarray, int, int, NDarray? , NDarray) ward_tree(NDarray X, NDarray? connectivity = null, int? n_clusters = null, bool return_distance = false)
            {
                ...
                return (ToCsharp<NDarray>(result[0]), ToCsharp<int>(result[1]), ToCsharp<int>(result[2]), ToCsharp<NDarray?>(result[3]), ToCsharp<NDarray>(result[4]));
            }
        }
    }
}